...
use rustc::mir::{Location, Lvalue, LvalueProjection, Mir, Operand, ProjectionElem, Rvalue};
use rustc::mir::{RETURN_POINTER};
...
use super::{LocationMap, MoveData};
use super::{MoveOut, MoveOutIndex, MovePath, MovePathIndex, MovePathLookup};
...
impl<'a, 'tcx> MoveDataBuilder<'a, 'tcx> {
    pub(super) fn new(mir: &'a Mir<'tcx>,
           tcx: TyCtxt<'a, 'tcx, 'tcx>,
           param_env: &'a ParameterEnvironment<'tcx>)
           -> Self {
    ...
    pub(super) fn finalize(self) -> MoveData<'tcx> {
        debug!("{}", {
...
impl<'a, 'tcx> MoveDataBuilder<'a, 'tcx> {
    pub(super) fn gather_statement(&mut self, loc: Location, stmt: &Statement<'tcx>) {
    ...
    pub(super) fn gather_terminator(&mut self, loc: Location, term: &Terminator<'tcx>) {
