...
impl<'b, 'a, 'tcx> Gatherer<'b, 'a, 'tcx> {
                                ...
                                -> Result<MovePathIndex, MoveError<'tcx>>
    {
        ...
        match (&proj.elem, &base_ty.sty) {
            (&ProjectionElem::Deref, &ty::TyRef(..)) |
            (&ProjectionElem::Deref, &ty::TyRawPtr(..)) =>
                return Err(MoveError::cannot_move_out_of(mir.source_info(self.loc).span,
                                                         BorrowedContent)),
            (&ProjectionElem::Deref, &ty::TyAdt(adt, _)) if adt.is_box() => {
                // okay, fall through (very special case)
            }
            ...
            (&ProjectionElem::Field(..), &ty::TyAdt(adt, _)) => {
                if adt.has_dtor(tcx) {
            ...
            (&ProjectionElem::Downcast(..), &ty::TyAdt(..)) => {
                // okay, fall through
            }

            (&ProjectionElem::Index(_), &ty::TySlice(elem_ty)) =>
                return Err(MoveError::cannot_move_out_of(
                    mir.source_info(self.loc).span,
                    InteriorOfSlice {
            ...
            (&ProjectionElem::Index(_), &ty::TyArray(elem_ty, _num_elems)) =>
                return Err(MoveError::cannot_move_out_of(
                    mir.source_info(self.loc).span,
                    InteriorOfArray {
            ...
            // Generated by slice patterns.

            (&ProjectionElem::ConstantIndex { .. }, &ty::TyArray(..)) => {
                // FIXME: is this broken or does drop elaboration actually handle it
                // without needing special cases?
            }

            // (&ProjectionElem::Subslice { .. }, &ty::TyArray(elem_ty)) |
            (&ProjectionElem::Subslice { .. }, &ty::TySlice(elem_ty)) => {
                return Err(MoveError::cannot_move_out_of(
                    mir.source_info(self.loc).span,
                    InteriorOfSlice { elem_ty, is_index: false }
                ));
            }


            (ref proj, ref sty) =>
                panic!("uncovered combination proj: {:?} and ty: {:?}", proj, sty),
        }
...
impl<'b, 'a, 'tcx> Gatherer<'b, 'a, 'tcx> {
    ...
    fn gather_move(&mut self, lval: &Lvalue<'tcx>) {
        ...
        let non_move_access = !lv_ty.moves_by_default(tcx, self.builder.param_env, DUMMY_SP);
        if exclude.type_is_copy && non_move_access {
        ...
        let path = match self.move_path_for(lval) {
            ...
            Err(error @ MoveError::IllegalMove { .. }) => {
                if non_move_access {
                    // Don't signal error if this is just a read.
                    return;
                } else {
