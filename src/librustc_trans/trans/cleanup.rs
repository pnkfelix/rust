...
impl<'blk, 'tcx> CleanupMethods<'blk, 'tcx> for FunctionContext<'blk, 'tcx> {
    ...
    fn push_ast_cleanup_scope(&self, debug_loc: NodeIdAndSpan) -> PendingAstCleanup {
        ...
        PendingAstCleanup(debug_loc.id)
    }

    fn push_loop_cleanup_scope(&self,
                               id: ast::NodeId,
                               exits: [Block<'blk, 'tcx>; EXIT_MAX]) -> PendingLoopCleanup {
        ...
        PendingLoopCleanup(id)
    }

    fn push_custom_cleanup_scope(&self) -> PendingCustomCleanup {
    ...
    fn pop_and_trans_ast_cleanup_scope(&self,
                                       bcx: Block<'blk, 'tcx>,
                                       cleanup_scope: PendingAstCleanup)
                                       -> Block<'blk, 'tcx> {
        let cleanup_scope = cleanup_scope.forget();
    ...
    fn pop_loop_cleanup_scope(&self,
                              cleanup_scope: PendingLoopCleanup) {
        let cleanup_scope = cleanup_scope.forget();
    ...
    fn pop_custom_cleanup_scope(&self,
                                custom_scope: PendingCustomCleanup) {
        let custom_scope = custom_scope.forget();
...
#[must_use] pub struct PendingAstCleanup(ast::NodeId);
#[must_use] pub struct PendingLoopCleanup(ast::NodeId);
#[must_use] pub struct PendingCustomCleanup(pub CustomScopeIndex);

impl PendingAstCleanup {
    fn forget(self) -> ast::NodeId { let r = self.0; ::std::mem::forget(self); r }
}
impl PendingLoopCleanup {
    fn forget(self) -> ast::NodeId { let r = self.0; ::std::mem::forget(self); r }
}
impl PendingCustomCleanup {
    fn forget(self) -> CustomScopeIndex { let r = self.0; ::std::mem::forget(self); r }
}

macro_rules! must_pop {
    ($id:ident) => {
        impl Drop for $id { fn drop(&mut self) { panic!("must pop {}", stringify!($id)); } }
    }
}

must_pop!(PendingAstCleanup);
must_pop!(PendingLoopCleanup);
must_pop!(PendingCustomCleanup);

pub trait CleanupMethods<'blk, 'tcx> {
    fn push_ast_cleanup_scope(&self, id: NodeIdAndSpan) -> PendingAstCleanup;
    fn push_loop_cleanup_scope(&self,
                               id: ast::NodeId,
                               exits: [Block<'blk, 'tcx>; EXIT_MAX]) -> PendingLoopCleanup;
    fn push_custom_cleanup_scope(&self) -> PendingCustomCleanup;
