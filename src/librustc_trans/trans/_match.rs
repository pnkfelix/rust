...
pub enum TransBindingMode {
    ...
    TrByMoveWithFresh(/* llbinding */ ValueRef),
    TrByMove,
    TrByRef,
}
...
impl<'tcx> Datum<'tcx, Lvalue> {
    fn match_input(&self) -> MatchInput {
        MatchInput {
            val: self.val,
            drop_flag_info: self.kind.drop_flag_info,
        }
    }
}
...
impl MatchInput {
    ...
    fn to_datum<'tcx>(self, ty: Ty<'tcx>) -> Datum<'tcx, Lvalue> {
        let kind = Lvalue::new_with_flag("MatchInput::to_datum",
                                         self.drop_flag_info);
        Datum::new(self.val, ty, kind)
    }
}
...
fn expand_nested_bindings<'a, 'p, 'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    m.iter().map(|br| {
        // FIXME do the bound_ptrs need to carry the drop-flags to
        // update as well?
...
fn compile_guard<'a, 'p, 'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    debug!("compile_guard(bcx={}, guard_expr={}, m={}, vals={})",
           ...
           m.repr(bcx.tcx()),
           vec_map_to_string(vals, |v| bcx.val_to_string(v.val)));
    let _indenter = indenter();
    ...
    match adt_vals {
        Some(field_vals) => {
            ...
            let mut vals: Vec<_> = field_vals.into_iter()
                .map(|v|MatchInput::from_val(v)).collect();
...
fn create_bindings_map<'blk, 'tcx>(bcx: Block<'blk, 'tcx>, pat: &ast::Pat,
    ...
    pat_bindings(&tcx.def_map, &*pat, |bm, p_id, span, path1| {
        ...
        let moves_by_default =
            ty::type_moves_by_default(&param_env, span, variable_ty);
        ...
        match bm {
            ast::BindByValue(_) if !moves_by_default || reassigned =>
            {
                llmatch = alloca_no_lifetime(bcx,
                                             ...
                                             "__llmatch");
                let llval = alloca_no_lifetime(bcx,
                                               llvariable_ty,
                                               &bcx.name(name));
                trmode = if !moves_by_default {
                    TrByCopy(llval)
                } else {
                    TrByMoveWithFresh(llval)
                };
            }
            ast::BindByValue(_) => {
...
pub fn store_arg<'blk, 'tcx>(mut bcx: Block<'blk, 'tcx>,
    ...
    match simple_identifier(&*pat) {
        ...
        None => {
            ...
            bind_irrefutable_pat(bcx, pat, arg.match_input(), arg_scope)
        }
    }
}
...
fn bind_irrefutable_pat<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    match pat.node {
        ...
        ast::PatVec(ref before, ref slice, ref after) => {
            let pat_ty = node_id_type(bcx, pat.id);
            let mut extracted =
                extract_vec_elems(bcx, pat_ty, before.len(), after.len(), val);
            match slice {
                &Some(_) => {
