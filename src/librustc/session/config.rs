...
options! {DebuggingOptions, DebuggingSetter, basic_debugging_options,
    ...
    nll_match_through_borrowed_temp: bool = (false, parse_bool, [UNTRACKED],
        "in match codegen, read via borrow of place rather than re-read via original place"),
    nll_emit_read_for_match: bool = (false, parse_bool, [UNTRACKED],
        "in match codegen, include abstract ReadForMatch statements (solely for mir-borrowck)"),
    nll_no_autoref_match_guard_bindings: bool = (false, parse_bool, [UNTRACKED],
        "in NLL match codegen, autoref bindings for guard and guards autoderef during execution"),
    nll_distinguish_byref_and_byval: bool = (false, parse_bool,  [UNTRACKED],
        "in match codegen, only `id` autorefed; `ref mut id` is `ref`, `ref id` is unchanged"),
