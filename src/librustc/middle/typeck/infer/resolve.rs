...
use floating::*;
use to_str::InferStr;
use middle::ty::{FnTyInfer, FnSig, FnTyBase};
...
impl ResolveState {
    ...
    fn resolve_type(&self, typ: ty::t) -> ty::t {
        ...
        match ty::get(typ).sty {
            ...
            ty::ty_infer(FnVar(ref f)) => {
                self.resolve_fn_var(f)
            }
            _ => {
    ...
    fn resolve_fn_var(&self, f: &FnTyInfer) -> ty::t {
        if !self.should(resolve_fnvar) {
            return ty::mk_fn_var(self.infcx.tcx, *f);
        }

        let node = self.infcx.get(&self.infcx.fn_var_bindings, f.meta);
        let meta = match node.possible_types {
            { ub:_, lb:Some(m) } => m,
            { ub:Some(m), lb:_ } => m,
            { ub:None, lb:None } => {
                if !self.should(force_fnvar) {
                    return ty::mk_fn_var(self.infcx.tcx, *f);
                }
