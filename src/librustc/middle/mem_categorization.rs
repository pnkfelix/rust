...
pub fn scope(tcx: &ty::ctxt, cmt: &cmt, item_scope_id: NodeId) -> ty::Region {
    match cmt.cat {
        cat_rvalue(temp_scope) => temp_scope,
        cat_upvar(..) => ty::ReScope(item_scope_id),
        ...
        cat_interior(ref cmt, _) |
        cat_discr(ref cmt, _) => scope(tcx, cmt, item_scope_id),
    }
}
