                       ...
                       loan_region: ty::Region)
    {
        ...
        match result {
            Ok(compute_loans::Loans(_, loans)) => {
                debug!("Result=Loans(%s)", loans.repr(self.tcx()));
                self.all_loans.push_all_move(loans);
            }

            Ok(compute_loans::Root(key, opt_loan, root_info)) => {
                debug!("Result=Root(key=%?, opt_loan=%s, root_info=%?)",
                       key, opt_loan.repr(self.tcx()), root_info);
                match opt_loan {
                    None => {}
                    Some(loan) => {
