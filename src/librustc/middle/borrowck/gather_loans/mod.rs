...
use syntax::ast::{expr,Block,fn_decl,NodeId,stmt,Local,pat};
...
impl Visitor<@mut GatherLoanCtxt> for GatherLoanVisitor {
    fn visit_expr(@mut self, a:@expr, env:@mut GatherLoanCtxt) {
        gather_loans_in_expr(self, a, env);
    }
    fn visit_block(@mut self, a:&Block, env:@mut GatherLoanCtxt) {
        gather_loans_in_block(self, a, env);
    }
    fn visit_fn(@mut self, a:&fn_kind, b:&fn_decl, c:&Block, d:span, e:NodeId, env:@mut GatherLoanCtxt) {
        gather_loans_in_fn(self, a, b, c, d, e, env);
    }
    fn visit_stmt(@mut self, a:@stmt, env:@mut GatherLoanCtxt) {
        add_stmt_to_map(self, a, env);
    }
    fn visit_pat(@mut self, a:@pat, env:@mut GatherLoanCtxt) {
        add_pat_to_id_range(self, a, env);
    }
    fn visit_local(@mut self, a:@Local, env:@mut GatherLoanCtxt) {
        gather_loans_in_local(self, a, env);
    }
}
