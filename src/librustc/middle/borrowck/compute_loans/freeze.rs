...
impl Freeze for ComputeLoansContext {
    ...
    {
        debug!("freeze(cmt=%s, loan_region=%?, pt=%?)",
        ...
        match cmt.cat {
            ...
            mc::cat_deref(cmt_base, _, mc::region_ptr(m, r)) => {
                freeze_borrowed_ptr(self, cmt, loan_region, pt,
                                    cmt_base, m, r)
            }

            mc::cat_deref(cmt_base, derefs, mc::gc_ptr(m)) => {
                freeze_managed_ptr(self, cmt, loan_region, pt,
                                   cmt_base, derefs, m)
            }
            ...
            mc::cat_discr(cmt_base, match_id) => {
                self.with_discr(match_id,
                                |c| c.freeze(cmt_base, loan_region, pt))
            }
        }
    }
}
    ...
    local_id: ast::node_id) -> BckResult<LoansOrRoot>
{
    ...
    self.loan_variable(cmt, loan_region, pt, MutLoan(m_imm), local_id)
}
