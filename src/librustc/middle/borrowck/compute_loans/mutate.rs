...
impl Mutate for ComputeLoansContext {
    ...
    {
        debug!("mutate(cmt=%s, loan_region=%?, totality=%?)",
        ...
        match cmt.cat {
            ...
            mc::cat_self(local_id) => {
                mutate_variable(self, cmt, loan_region, totality, local_id)
            }

            mc::cat_interior(cmt_base, f @ mc::interior_field(_, m)) |
            mc::cat_interior(cmt_base, f @ mc::interior_index(_, m)) => {
                mutate_owned_content(self, cmt, loan_region, totality,
            ...
            mc::cat_deref(cmt_base, _, mc::region_ptr(m, r)) => {
                mutate_borrowed_ptr(self, cmt, loan_region, totality,
                                    cmt_base, m, r)
            }

            mc::cat_deref(cmt_base, derefs, mc::gc_ptr(m)) => {
                mutate_managed_ptr(self, cmt, loan_region, totality,
                                   cmt_base, derefs, m)
            }
            ...
            mc::cat_discr(cmt_base, match_id) => {
                self.with_discr(match_id,
                                |c| c.mutate(cmt_base, loan_region, totality))
            }

            mc::cat_stack_upvar(cmt) => {
                self.mutate(cmt, loan_region, totality)
            }
    ...
    local_id: ast::node_id) -> BckResult<LoansOrRoot>
{
    ...
    if_ok!(self.check_mutable(cmt));
    self.loan_variable(cmt, loan_region, MutLoan(m_mutbl, totality), local_id)
}
    ...
    lp_elem: LoanPathElem) -> BckResult<LoansOrRoot>
{
    match content_mutbl {
        ast::m_imm => {
            ...
            let loans =
                if_ok!(self.mutate(cmt_base, loan_region, Partial));
            Ok(self.add_loan(loans, cmt, loan_region,
                             MutLoan(m_mutbl, totality), lp_elem))
        }
    ...
    pointer_region: ty::Region) -> BckResult<LoansOrRoot>
{
    ...
    self.mutate_or_freeze_mut_borrowed_ptr(
        cmt, loan_region, cmt_base, pointer_mutbl, pointer_region,
        MutLoan(m_mutbl, totality))
}
    ...
    pointer_mutbl: ast::mutability) -> BckResult<LoansOrRoot>
{
    ...
    self.mutate_or_freeze_mut_managed_ptr(
        cmt, loan_region, DynaMut, cmt_base, derefs, pointer_mutbl)
}