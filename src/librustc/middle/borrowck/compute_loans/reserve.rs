...
impl Reserve for ComputeLoansContext {
    ...
    {
        debug!("reserve(cmt=%s, loan_region=%?, reason=%?)",
               cmt.repr(self.tcx()), loan_region, reason);
        let _i = indenter();
    ...
    local_id: ast::node_id) -> BckResult<LoansOrRoot>
{
    ...
    self.loan_variable_without_checking_scope(
        cmt, loan_region, ReserveLoan, local_id)
}
    ...
    lp_elem: LoanPathElem) -> BckResult<LoansOrRoot>
{
    ...
    Ok(self.add_loan(loans, cmt, loan_region, ReserveLoan, lp_elem))
}
    ...
    match pointer_mutbl {
        ...
        m_mutbl => {
            ...
            let loans = if_ok!(self.reserve(cmt_base, loan_region,
                                            reason));
            Ok(self.add_loan(loans, cmt, loan_region, ReserveLoan, LpDeref))
        }
    }
}
