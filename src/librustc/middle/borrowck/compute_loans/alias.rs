             ...
             totality: PartialTotal) -> BckResult<LoansOrRoot>
    {
        ...
        match cmt.cat {
            ...
            mc::cat_self(local_id) => {
                alias_variable(self, cmt, loan_region, totality, local_id)
            }

            mc::cat_interior(cmt_base, f @ mc::interior_field(*)) |
            mc::cat_interior(cmt_base, f @ mc::interior_index(*)) => {
                alias_owned_interior(self, cmt, loan_region, totality,
            ...
            mc::cat_deref(cmt_base, _, mc::uniq_ptr(_)) => {
                alias_owned_pointer(self, cmt, loan_region,
                                    totality, cmt_base)
            }

            mc::cat_deref(_, _, mc::region_ptr(_, r)) => {
                alias_borrowed_ptr(self, cmt, loan_region, totality, r)
            }
            ...
            mc::cat_discr(cmt_base, match_id) => {
                self.with_discr(match_id,
                                |c| c.alias(cmt_base, loan_region, totality))
            }
    ...
    local_id: ast::node_id) -> BckResult<LoansOrRoot>
{
    ...
    self.loan_variable(cmt, loan_region,
                       MutLoan(ast::m_const, totality), local_id)
}
    ...
    lp_elem: LoanPathElem) -> BckResult<LoansOrRoot>
{
    ...
    Ok(self.add_loan(loans, cmt, loan_region, MutLoan(ast::m_const, totality),
                     lp_elem))
}
