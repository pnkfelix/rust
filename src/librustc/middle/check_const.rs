...
impl Visitor<bool> for CheckCrateVisitor {
    fn visit_item(@mut self, a:@item, env:bool) {
        check_item(self, self.sess, self.ast_map, self.def_map, a, env);
    }
    fn visit_pat(@mut self, a:@pat, env:bool) {
        check_pat(self, a, env);
    }
    fn visit_expr(@mut self, a:@expr, env:bool) {
        check_expr(self, self.sess, self.def_map, self.method_map, self.tcx, a, env);
    }
}
...
pub fn check_item_recursion(sess: Session,
    ...
    let visitor = @mut CheckItemRecursionVisitor;
