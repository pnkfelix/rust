...
tag unsafe_ty { contains(ty::t); mutbl_contains(ty::t); }
...
fn check_call(cx: ctx, sc: scope, f: @ast::expr, args: [@ast::expr])
    ...
    let mutbl_roots: [{arg: uint, node: node_id}] = [];
    let bindings = [];
    let i = 0u;
    ...
    for {node: node, arg: arg} in mutbl_roots {
        let i = 0u;
        for b in bindings {
            if i != arg {
                alt b.root_var {
...
fn check_for(cx: ctx, local: @ast::local, seq: @ast::expr, blk: ast::blk,
    ...
    alt ty::struct(cx.tcx, seq_t) {
      ty::ty_vec(mt) { // note: not a const(vec(t))
...
fn ty_can_unsafely_include(cx: ctx, needle: unsafe_ty, haystack: ty::t,
    ...
    fn helper(tcx: ty::ctxt, needle: unsafe_ty, haystack: ty::t, mutbl: bool)
        ...
        alt ty::struct(tcx, haystack) {
          ...
          ty::ty_const(ty) {
            be helper(tcx, needle, ty, false);
          }
          ty::ty_box(ty) | ty::ty_ptr(ty) | ty::ty_uniq(ty) {
            be helper(tcx, needle, ty, mutbl);
          }
          ty::ty_rec(fields) {
...
fn copy_is_expensive(tcx: ty::ctxt, ty: ty::t) -> bool {
    fn score_ty(tcx: ty::ctxt, ty: ty::t) -> uint {
        ret alt ty::struct(tcx, ty) {
          ...
          ty::ty_uniq(ty) { 1u + score_ty(tcx, ty) }
...
type pattern_root = {id: node_id,
                     name: ident,
                     mutbl: option::t<unsafe_ty>,
                     span: span};

fn pattern_roots(tcx: ty::ctxt, mutbl: option::t<unsafe_ty>, pat: @ast::pat)
    -> [pattern_root] {
    fn walk(tcx: ty::ctxt, mutbl: option::t<unsafe_ty>, pat: @ast::pat,
            &set: [pattern_root]) {
        alt pat.node {
          ast::pat_wild. | ast::pat_lit(_) | ast::pat_range(_, _) {}
          ast::pat_bind(nm, sub) {
            set += [{id: pat.id, name: nm, mutbl: mutbl, span: pat.span}];
          ...
          ast::pat_rec(fs, _) {
            ...
            for f in fs {
                let m = ty::get_field(tcx, ty, f.ident).mt.mutbl != ast::imm;
                walk(tcx, m ? some(contains(ty)) : mutbl, f.pat, set);
            }
          }
          ast::pat_box(p) {
            let ty = ty::node_id_to_type(tcx, pat.id);
            let m = alt ty::struct(tcx, ty) {
              ty::ty_box(mt) { mt.mutbl != ast::imm }
            };
            walk(tcx, m ? some(contains(ty)) : mutbl, p, set);
          }
          ast::pat_uniq(p) {
            let ty = ty::node_id_to_type(tcx, pat.id);
            let m = alt ty::struct(tcx, ty) {
              ty::ty_uniq(mt) { mt.mutbl != ast::imm }
            };
            walk(tcx, m ? some(contains(ty)) : mutbl, p, set);
          }
        }
...
type deref = @{mutbl: bool, outer_t: ty::t};
...
// ds=[index,field])
fn base_expr_root(tcx: ty::ctxt, ex: @ast::expr, autoderef: bool) ->
   {ex: @ast::expr, ds: @[deref]} {
    fn maybe_auto_unbox(tcx: ty::ctxt, t: ty::t) -> {t: ty::t, ds: [deref]} {
        let ds = [], t = t, is_const = false;
        while true {
            alt ty::struct(tcx, t) {
              ty::ty_const(subty) {
                t = subty;
    ...
    while true {
        alt copy ex.node {
          ast::expr_field(base, ident, _) {
            ...
            ds += [@{mutbl: is_mut, outer_t: auto_unbox.t}];
            ds += auto_unbox.ds;
            ex = base;
          }
          ast::expr_index(base, _) {
            ...
            alt ty::struct(tcx, auto_unbox.t) {
              ty::ty_vec(mt) {
                ds +=
                    [@{mutbl: mt.mutbl == ast::mutbl,
                       outer_t: auto_unbox.t}];
              }
              ty::ty_str. {
                ds += [@{mutbl: false, outer_t: auto_unbox.t}];
              }
            }
            ds += auto_unbox.ds;
            ex = base;
          ...
          ast::expr_unary(ast::deref., base) {
            ...
            alt ty::struct(tcx, base_t) {
              ty::ty_box(mt) { is_mut = mt.mutbl == ast::mutbl; }
              ty::ty_uniq(mt) { is_mut = mt.mutbl == ast::mutbl; }
              ty::ty_res(_, _, _) { }
              ty::ty_tag(_, _) { }
              ty::ty_ptr(mt) { is_mut = mt.mutbl == ast::mutbl; }
            }
            ds += [@{mutbl: is_mut, outer_t: base_t}];
            ex = base;
          }
          _ { break; }
        }
...
// Wraps base_expr_root to also handle roots that exist through
// return-by-reference
fn expr_root(cx: ctx, ex: @ast::expr, autoderef: bool)
    -> {ex: @ast::expr, mutbl: option::t<unsafe_ty>} {
    let base_root = base_expr_root(cx.tcx, ex, autoderef);
    let unsafe_ty = none;
    ...
    alt base_root.ex.node {
      ast::expr_path(_) {
        alt cx.tcx.def_map.get(base_root.ex.id) {
          ast::def_obj_field(_, ast::mutbl.) {
            let m_c = mutbl_contains(ty::expr_ty(cx.tcx, base_root.ex));
            unsafe_ty = some(m_c);
          }
          _ {}
        }
