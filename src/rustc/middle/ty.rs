...
fn expr_is_lval(tcx: ctxt,
                ...
                e: @ast::expr) -> bool {
    expr_kind(tcx, method_map, e) == LvalueExpr
}
...
fn note_and_explain_type_err(cx: ctxt, err: &type_err) {
    match *err {
        ...
        terr_regions_not_same(region1, region2) => {
            ...
            note_and_explain_region(cx, ~"...is not the same lifetime as ", region2, ~"");
        }
        terr_regions_no_overlap(region1, region2) => {
            note_and_explain_region(cx, ~"", region1, ~"...");
            note_and_explain_region(cx, ~"...does not overlap ", region2, ~"");
        }
        _ => {}
    }
}
