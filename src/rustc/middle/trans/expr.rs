...
import ty::class_items_as_mutable_fields;
...
fn trans_into(bcx: block, expr: @ast::expr, dest: Dest) -> block {
    ...
    match kind {
        ty::LvalueExpr => {
            ...
            return datumblock.store_to_dest(dest);
        }
        ty::RvalueDatumExpr => {
...
fn trans_rvalue_datum(bcx: block, expr: @ast::expr) -> DatumBlock {
    ...
    match expr.node {
        ...
        ast::expr_unary(op, x) => {
            return trans_unary_imm(bcx, expr, op, x);
        }
...
fn trans_unary_imm(bcx: block,
                   un_expr: @ast::expr,
                   op: ast::unop,
                   sub_expr: @ast::expr) -> DatumBlock {

    let _icx = bcx.insn_ctxt("trans_unary_imm");
                ...
                rhs: @ast::expr) -> DatumBlock
{
    ...
    do base::with_scope_datumblock(bcx, binop_expr.info(),
                                   ~"trans_binary") |bcx| {
        match op {
            ast::and => {
...
enum cast_kind {
    cast_pointer, cast_integral, cast_float, cast_enum, cast_other
}
