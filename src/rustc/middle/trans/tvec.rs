...
import expr::{Dest, SaveIn, Ignore};
import datum::*;
        ...
        ast::expr_repeat(element, count_expr, _) => {
            match dest {
                ...
                SaveIn(lldest) => {
                    ...
                    //
                    // NB---We do not add this space to the list of temp
                    // cleanups.  This is because its cleanup will be revoked
                    // when we move it into the array at the end.
                    let lltmpspace = base::alloca(bcx, vt.llunit_ty);
                    bcx = expr::trans_into(bcx, element, SaveIn(lltmpspace));
                    add_clean_temp_mem(bcx, lltmpspace, vt.unit_ty);
                    let tmpdatum = Datum {val: lltmpspace,
                                          ty: vt.unit_ty,
                                          mode: ByRef,
                                          source: FromRvalue};
                    ...
                    for uint::range(0, count) |i| {
                        ...
                        if i < count - 1 {
                            ...
                            // Move the last one in.  This also cancels the
                            // tmpdatum cleanup.
