...
impl Datum {
    ...
    /// A helper for `copy_to()` which does not check to see if we are copying
    /// to/from the same value.
    ...
    fn to_ref_llval(bcx: block) -> ValueRef {
        match self.mode {
            ByRef => self.val,
            ByValue => {
                assert ty::type_is_immediate(self.ty);
    ...
    fn to_appropriate_llval(bcx: block) -> ValueRef {
        ...
        if ty::type_is_immediate(self.ty) {
...
impl DatumBlock {
    ...
    fn store_to_dest(dest: expr::Dest) -> block {
        self.datum.store_to_dest(self.bcx, dest)
    }
...
/*
impl DatumCallee {
    fn to_result(sp: span) -> LvalueResult {
        match self.env {
            Closure => self.res,
            NotCallable => self.res,
            FnItem => {
                // convert from a raw fn ptr to a (fnptr, NULL) pair
                let bcx = self.res.bcx;
                let llfnty = llvm::LLVMGetElementType(val_ty(self.val));
                let pair = alloca(bcx, T_fn_pair(bcx.ccx(), llfnty));
                let llenvptr = null_env_ptr(bcx);
                fill_fn_pair(bcx, pair, llfn, llenvptr);
                LvalueResult {
                    lv: Lvalue {val: llfn,
                                ty: self.res.lv.ty,
                                mode: ByRef,
            ...
            SelfEnv(*) => {
                self.res.bcx.sess().span_bug(
                    sp,
                    ~"method reference used as value");
            }
        }
    }
}
