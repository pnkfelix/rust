...
fn typeck_tables_of_with_fallback<'tcx>(
    ...
    let tables = Inherited::build(tcx, def_id).enter(|inh| {
        ...
        let fcx = if let (Some(header), Some(decl)) = (fn_header, fn_decl) {
            ...
            GatherLocalsVisitor { fcx: &fcx, parent_id: id, within_fn_param: false }.visit_body(body);
...
fn check_fn<'a, 'tcx>(
    ...
    *fcx.ps.borrow_mut() = UnsafetyState::function(fn_sig.unsafety, fn_id);
    ...
    GatherLocalsVisitor { fcx: &fcx, parent_id: outer_hir_id, within_fn_param: false }.visit_body(body);
