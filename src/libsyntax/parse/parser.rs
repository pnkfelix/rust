...
impl<'a> Parser<'a> {
    ...
    pub fn parse_trait_items(&mut self) -> PResult<'a,  Vec<TraitItem>> {
        self.parse_unspanned_seq(
            ...
            let (name, node) = if p.eat_keyword(keywords::Type) {
                let TyParam {ident, bounds, default, ..} = p.parse_ty_param(vec![])?;
                p.expect(&token::Semi)?;
    ...
    /// error if we encounter attribues after the bound lifetimes.
    ///
        ...
        loop {
            ...
            match self.token {
                token::Lifetime(_) => {
                    ...
                    res.push(ast::LifetimeDef { attrs: attrs.into_thin_attrs(),
                                                lifetime: lifetime,
                                                bounds: bounds });
                }
    ...
    fn parse_ty_param(&mut self, preceding_attrs: Vec<ast::Attribute>) -> PResult<'a, TyParam> {
        ...
        Ok(TyParam {
            attrs: preceding_attrs.into_thin_attrs(),
            ident: ident,
            id: ast::DUMMY_NODE_ID,
            bounds: bounds,
            default: default,
