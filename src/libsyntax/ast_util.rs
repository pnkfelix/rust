...
impl Visitor<()> for IdVisitor {
    fn visit_mod(@mut self,
        ...
        visit::walk_mod(self as @Visitor<()>, module, env)
    }
    ...
    fn visit_view_item(@mut self, view_item: &view_item, env: ()) {
        ...
        visit::walk_view_item(self as @Visitor<()>, view_item, env)
    }

    fn visit_foreign_item(@mut self, foreign_item: @foreign_item, env: ()) {
        (self.visit_callback)(foreign_item.id);
        visit::walk_foreign_item(self as @Visitor<()>, foreign_item, env)
    }
    ...
    fn visit_item(@mut self, item: @item, env: ()) {
        ...
        visit::walk_item(self as @Visitor<()>, item, env);
    ...
    fn visit_local(@mut self, local: @Local, env: ()) {
        ...
        visit::walk_local(self as @Visitor<()>, local, env)
    }

    fn visit_block(@mut self, block: &Block, env: ()) {
        (self.visit_callback)(block.id);
        visit::walk_block(self as @Visitor<()>, block, env)
    }

    fn visit_stmt(@mut self, statement: @stmt, env: ()) {
        (self.visit_callback)(ast_util::stmt_id(statement));
        visit::walk_stmt(self as @Visitor<()>, statement, env)
    }

    // XXX: Default
    fn visit_arm(@mut self, arm: &arm, env: ()) {
        visit::walk_arm(self as @Visitor<()>, arm, env)
    }

    fn visit_pat(@mut self, pattern: @pat, env: ()) {
        (self.visit_callback)(pattern.id);
        visit::walk_pat(self as @Visitor<()>, pattern, env)
    }

    // XXX: Default
    fn visit_decl(@mut self, declaration: @decl, env: ()) {
        visit::walk_decl(self as @Visitor<()>, declaration, env)
    }
    ...
    fn visit_expr(@mut self, expression: @expr, env: ()) {
        ...
        visit::walk_expr(self as @Visitor<()>, expression, env)
    }
    ...
    fn visit_ty(@mut self, typ: &Ty, env: ()) {
        ...
        visit::walk_ty(self as @Visitor<()>, typ, env)
    }

    fn visit_generics(@mut self, generics: &Generics, env: ()) {
        self.visit_generics_helper(generics);
        visit::walk_generics(self as @Visitor<()>, generics, env)
    }
    ...
    fn visit_ty_method(@mut self, type_method: &TypeMethod, env: ()) {
        visit::walk_ty_method(self as @Visitor<()>, type_method, env)
    }

    // XXX: Default
    fn visit_trait_method(@mut self, trait_method: &trait_method, env: ()) {
        visit::walk_trait_method(self as @Visitor<()>, trait_method, env)
    }
    ...
    fn visit_struct_field(@mut self, struct_field: @struct_field, env: ()) {
        ...
        visit::walk_struct_field(self as @Visitor<()>, struct_field, env)
    }
}
...
impl EachViewItem for ast::Crate {
    fn each_view_item(&self, f: @fn(&ast::view_item) -> bool) -> bool {
        ...
        visit::walk_crate(visitor as @Visitor<()>, self, ());
        true
    }
}
