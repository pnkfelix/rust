...
pub fn visit_fn<E:Clone>(fk: &fn_kind, decl: &fn_decl, body: &blk, sp: span,
                         id: node_id, e_vt: (E, vt<E>)) {
    walk_fn(&e_vt, fk, decl, body, sp, id)
}

pub fn walk_fn<V:Visitor>(v:&V, fk: &fn_kind, decl: &fn_decl, body: &blk,
...
pub fn visit_block<E:Clone>(b: &blk, e_vt: (E, vt<E>)) {
    walk_block(&e_vt, b)
}

pub fn walk_block<V:Visitor>(v:&V, b: &blk) {
...
pub fn mk_simple_visitor(v: simple_visitor) -> vt<()> {
    ...
    fn v_block(f: @fn(&ast::blk), bl: &ast::blk, e_vt: ((), vt<()>)) {
        f(bl);
        visit_block(bl, e_vt);
    }
