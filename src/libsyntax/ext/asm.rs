        ...
        match state {
            ...
            Outputs => {
                while *p.token != token::EOF &&
                    ...
                    let (constraint, str_style) = p.parse_str();
                    if str_style != ast::CookedStr {
                        p.span_err(*p.last_span, "asm output can't be raw string");
                    }
            ...
            Inputs => {
                while *p.token != token::EOF &&
                    ...
                    let (constraint, str_style) = p.parse_str();
                    if str_style != ast::CookedStr {
                        p.span_err(*p.last_span, "asm input can't be raw string");
                    }
                    p.expect(&token::LPAREN);
            ...
            Clobbers => {
                ...
                while *p.token != token::EOF &&
                    ...
                    let (s, str_style) = p.parse_str();
                    if str_style != ast::CookedStr {
                        p.span_err(*p.last_span, "asm clobbers can't be raw string");
                    }
                    let clob = format!("~\\{{}\\}", s);
                    clobs.push(clob);
                }
            ...
            Options => {
                let (option, str_style) = p.parse_str();
                if str_style != ast::CookedStr {
                    p.span_err(*p.last_span, "asm options can't be raw string");
                }
