...
pub fn path_to_str_with_sep(p: &[path_elt], sep: &str, itr: @ident_interner)
    ...
    let strs = do p.map |e| {
        match *e {
          path_mod(s) | path_name(s) => itr.get(s.name).unwrap(),
        }
    };
    strs.connect(sep)
}
...
pub fn path_ident_to_str(p: &path, i: ident, itr: @ident_interner) -> ~str {
    if p.is_empty() {
        ...
        itr.get(i.name).unwrap().to_owned()
    } else {
        fmt!("%s::%s", path_to_str(*p, itr), itr.get(i.name).unwrap())
    }
}
...
pub fn path_elt_to_str(pe: path_elt, itr: @ident_interner) -> ~str {
    match pe {
        path_mod(s) | path_name(s) => itr.get(s.name).unwrap().to_owned(),
    }
}
    ...
    match map.find(&id) {
      ...
      Some(&node_method(m, _, path)) => {
        fmt!("method %s in %s (id=%?)",
             itr.get(m.ident.name).unwrap(), path_to_str(*path, itr), id)
      }
      ...
      Some(&node_variant(ref variant, _, path)) => {
        fmt!("variant %s in %s (id=%?)",
             itr.get(variant.node.name.name).unwrap(), path_to_str(*path, itr), id)
      }
      Some(&node_expr(expr)) => {
      ...
      Some(&node_local(ident)) => {
        fmt!("local (id=%?, name=%s)", id, itr.get(ident.name).unwrap())
      }
      Some(&node_block(_)) => {
        fmt!("block")
      }
