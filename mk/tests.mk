...
check: cleantestlibs cleantmptestlogs tidy all check-stage2
	$(Q)$(C) $(CFG_PYTHON) $(S)src/etc/check-summary.py tmp/*.log
...
cleantestlibs:
	$(Q)$(C) find $(CFG_BUILD_TRIPLE)/test \
         -name '*.[odasS]' -o \
         -name '*.so' -o      \
         -name '*.dylib' -o   \
         -name '*.dll' -o     \
...
tidy:
		...
		$(Q)$(C) find $(S)src -name '*.r[sc]' \
		| grep '^$(S)src/test' -v \
		| xargs -n 10 $(CFG_PYTHON) $(S)src/etc/tidy.py
		$(Q)$(C) find $(S)src/etc -name '*.py' \
		| xargs -n 10 $(CFG_PYTHON) $(S)src/etc/tidy.py
		$(Q)$(C) echo $(ALL_CS) \
		| xargs -n 10 $(CFG_PYTHON) $(S)src/etc/tidy.py
		$(Q)$(C) echo $(ALL_HS) \
...
$$(call TEST_OK_FILE,$(1),$(2),$(3),$(4)): \
	...
	@$$(call E, run: $$<)
	$$(Q)$(C) $$(call CFG_RUN_TEST_$(2),$$<,$(2),$(3)) $$(TESTARGS)	\
...
doc-$(2)-extract$(1):
	...
	$$(Q)$(C) rm -f $(1)/test/doc-$(2)/*.rs
	$$(Q)$(C) $$(EXTRACT_TESTS) $$(S)doc/$(2).md $(1)/test/doc-$(2)
...
tmp/$(FT).rc tmp/$(FT_DRIVER).rs: \
	...
	$(Q)$(C) $(CFG_PYTHON) $(S)src/etc/combine-tests.py
