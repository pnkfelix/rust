...
use rustc::middle::ty;
use rustc::middle::ty::{BoundRegion, FreeRegion, Region, RegionVid, Vid};
use rustc::middle::ty::{ReEmpty, ReStatic, ReInfer, ReFree, ReEarlyBound,
                        ReLateBound};
use rustc::middle::ty::{ReScope, ReVar, ReSkolemized, BrFresh};
use rustc::middle::typeck::infer::cres;
use rustc::middle::typeck::infer::{RegionVariableOrigin, SubregionOrigin, TypeTrace};
use rustc::middle::typeck::infer;
use rustc::middle::graph;
use rustc::middle::graph::{Direction, NodeIndex};
use rustc::util::common::indenter;
...
fn regions(bounds: Vec<RegionAndOrigin>) -> Vec<Region> {
    bounds.iter().map(|x| x.region).collect::<Vec<Region>>()
}
        ...
        errors: &mut Vec<RegionResolutionError>)
    {
        ...
        self.tcx.sess.span_bug(
            ...
            format!("collect_error_for_expanding_node() could not find error \
                 ...
                 node_idx,
                 regions(lower_bounds).repr(self.tcx),
                 regions(upper_bounds).repr(self.tcx)));
    }
